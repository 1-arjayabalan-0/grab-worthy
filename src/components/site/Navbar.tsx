...
export function Navbar() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Nav */}
      <div
        className={`md:hidden absolute top-full left-0 w-full bg-background border-b border-border overflow-hidden transition-all duration-300 ${
          mobileMenuOpen ? 'max-h-[70vh] border-t' : 'max-h-0 border-t-0'
        }`}
      >
        <div className="flex flex-col p-4 gap-4">
          <Link href="/blog" className="text-lg font-serif">All Articles</Link>
          <div className="h-px bg-border/50" />
          <div className="text-sm text-muted-foreground uppercase tracking-wider mb-2">Categories</div>
          ...
          <Link href="/contact" className="text-lg font-serif">Contact</Link>
        </div>
      </div>
    </header>
  );
